            ...
            {chapters.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
                {chapters.map((ch) => {
                  ...
                  const isSamplePapers = ch.slug === 'sample-question-papers' || ch.title.toLowerCase().includes('sample question papers');
                  ...
                  if (!hasQuestions) {
                    if (isSamplePapers) {
                      return (
                        <Card 
                          key={ch.id}
                          className="p-6 bg-gradient-to-br from-indigo-50 to-blue-50 border-2 border-indigo-100 rounded-2xl hover:shadow-lg transition-all duration-300"
                        >
                          <div className="flex flex-col items-center justify-center py-8 space-y-4 text-center">
                            <div className="w-14 h-14 rounded-2xl bg-white shadow-inner border border-indigo-100 flex items-center justify-center text-2xl">
                              📄
                            </div>
                            <div>
                              <h3 className="text-xl font-bold text-gray-900 mb-1">{ch.title}</h3>
                              <p className="text-sm text-gray-600">Contains 5 sample papers for practice</p>
                            </div>
                            <div className="flex items-center gap-2 text-sm text-indigo-700 bg-white px-3 py-1 rounded-full border border-indigo-100">
                              <span className="text-lg">🧾</span>
                              <span>Sample papers</span>
                            </div>
                            <button
                              onClick={() => navigate(`/sample-papers/${subjectSlug}/${bookSlug}`)}
                              className="mt-2 inline-flex items-center justify-center px-4 py-2.5 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition-colors shadow-md"
                            >
                              View Sample Papers
                            </button>
                          </div>
